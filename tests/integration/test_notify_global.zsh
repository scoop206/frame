#!/usr/bin/env zsh
# frame notify's global banner switch (frame notifications off|on): the `notify`
...
test_toggle_never_banners() {
  # `frame notifications on` is the toggle, not a banner that says "on".
  setup_shell_frame
  run_frame notifications on
  assert_status 0
  assert_file_absent "$FAKE_OSASCRIPT_LOG"
...
test_notify_args_are_a_usage_error() {
  ...
  # spellings) error and point at frame notifications, without a banner.
  setup_shell_frame
  run_frame notify pssst
  assert_status 2
  assert_contains "$OUT" "frame notifications"
  assert_file_absent "$FAKE_OSASCRIPT_LOG"
  run_frame notify off
  assert_status 2
}

test_notifications_needs_a_subcommand() {
  setup_shell_frame
  run_frame notifications
  assert_status 2
  assert_contains "$OUT" "usage: frame notifications"
  run_frame notifications pssst
  assert_status 2
  assert_file_absent "$FAKE_OSASCRIPT_LOG"
}
