...
# user-facing as `frame notifications init` — the badge setup/repair command.
#
...
# bundle and run frame notifications init.)
...
if ! SRC=$(brew --prefix terminal-notifier 2>/dev/null) || [[ ! -d "$SRC" ]]; then
  ...
  echo "    brew install terminal-notifier   # then: frame notifications init" >&2
  exit 1
fi
...
echo "  No banner?"
echo ""
echo "  test for notification with:"
echo "  frame notify"
echo ""
echo "  test while performing these steps:"
echo ""
echo " 1. Enable at System Settings → Notifications → Frame"
echo ""
echo " 2. The notification daemons cached the old bundle — run:"
echo "    killall usernoted NotificationCenter"
echo "  (both respawn instantly)."
echo ""
echo "  Clicking on Notification asks for Accessibility on first click — grant that too."
